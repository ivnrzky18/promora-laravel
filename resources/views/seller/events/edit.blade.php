...
@section('title', 'Edit Event - Promora')
...
<div class="max-w-3xl space-y-6">
    ...
    <div class="flex items-center space-x-3">
        ...
        <div>
            ...
            <p class="text-gray-500 mt-0.5 text-sm">Perbarui informasi event Anda</p>
        </div>
    </div>

    {{-- Form --}}
    <form method="POST" action="{{ route('seller.events.update', $event) }}" enctype="multipart/form-data"
            ...
            <div class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4"
                              class="w-full px-3 py-2.5 min-h-[44px] border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent @error('description') border-red-400 @enderror">{{ old('description', $event->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-orange-400 hover:bg-orange-50 transition-colors">
                        <div class="flex flex-col items-center justify-center pt-4 pb-4">
                            ...
                            <p class="text-sm text-gray-500">{{ $event->poster_image ? 'Ganti poster' : 'Upload poster' }}</p>
